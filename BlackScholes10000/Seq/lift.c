...
void liftKernel(float* v__4, Tuple_float_float* v__6){ 
{
  /* Static local memory */
  /* Typed Value memory */
  /* Private Memory */
  /* map_seq */
  for (int v_i_3 = 0;v_i_3<10000;v_i_3 = (1 + v_i_3)){
    v__6[v_i_3] = blackScholesComp(v__4[v_i_3]);
  }
  /* end map_seq */
}}
...
//code for parameter p176
float *p176 = malloc(sizeof(float) * 10000);
//geerating array of type float
for(unsigned long i1 = 0; i1 < 10000; i1++){
inputData = strtok(NULL, "_"); sscanf(inputData,"%f",&(p176[(i1 * 10000/10000)]));
}
//main invocation
Tuple_float_float *output = malloc(sizeof(Tuple_float_float) * 10000);
struct timeval startTime;
gettimeofday(&startTime, NULL);
liftKernel( p176, output);
