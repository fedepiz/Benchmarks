...
float *p174 = malloc(sizeof(float) * 128 * 128);
//geerating array of type float
for(unsigned long i2 = 0; i2 < 128; i2++){
for(unsigned long i3 = 0; i3 < 128; i3++){
inputData = strtok(NULL, "_"); sscanf(inputData,"%f",&(p174[(i2 * 128 * 128/128) + (i3 * 128 * 128/128/128)]));
}
}
//code for parameter p10
float *p10 = malloc(sizeof(float) * 128 * 128);
//geerating array of type float
for(unsigned long i6 = 0; i6 < 128; i6++){
for(unsigned long i7 = 0; i7 < 128; i7++){
inputData = strtok(NULL, "_"); sscanf(inputData,"%f",&(p10[(i6 * 128 * 128/128) + (i7 * 128 * 128/128/128)]));
}
}
